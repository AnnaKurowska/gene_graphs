...
Get5LSstart <- function(gene, gffdf) {
  gffdf %>% 
...
GetGeneDatamatrix5LS <- function(gene, dataset, hdf5file, gffdf, nnt_gene) {
  data_mat_all <- GetGeneDatamatrix(gene, dataset, hdf5file)
  n_left5 <- Get5LSstart(gene, gffdf) # column to start from (5'end)
...
plotting_5LS<- function(input_data) {
...
plotting_multiple <- function(input_data, gene_names) {
  purrr :: map(input_data, plotting_5LS) %>%
...
## sum_uAUG and sum_uAUG_mulitple used in CDS_5LS and all_regions_together
...
CDS_5LS <- function(genes, gene_names) {
...
## bar plot showing AUG and 5LS for individual genes, requires separate output from CDS_5LS
efficiency_barplot_each_gene <- function(CDS_5LS_AUG_data, gene){
  positions <- c( "5'LS", "AUG")
  value <-filter(data, genes == gene)
  # 
  ggplot(CDS_5LS_AUG_data, aes(x = region, y = count, fill = region)) +
...
# efficiency bar plot (5LS and AUG) for all input genes, includes the CDS_5LS step; input: GetGeneCodonPosReads1dsnap,A_mapped_genes (1 gene only) or All_genesAmapped
efficiency_barplot <- function(genes, gene_names) {
  CDS_5LS_data <-CDS_5LS(genes, gene_names) %>%
    tidyr::gather( key = "region", value = "count", -genes)
  CDS_5LS_data$count <- as.numeric(CDS_5LS_data$count)
  
  positions <- c("5'LS", "AUG")
  
  ggplot(CDS_5LS_data, aes(x=region, y = count, fill = region )) +
    geom_col(show.legend = FALSE) +
    theme_classic() +
    labs(y= "CPMB", x = "Region (mRNA)") +
    scale_y_continuous(expand = c(0,0)) +
...
compared_LS_single<-function(condition1, condition2, condition3, gene){
  ...
  search_for_5LS <- function(condition, gene){
    condition %>%
      filter(Gene == gene ) %>%
      select(Counts_LS5) %>%
      pull
  ...
  LS1 <-search_for_5LS(condition1, gene)
  
  LS2 <-search_for_5LS(condition2, gene)
  
  LS3 <-search_for_5LS(condition3, gene)
...
finding_correlation_preparation<- function(gene_VEG, gene_Anaph, gene_Spore){
  ...
  comparing_genes_stages_5LS <-comparing_genes_stages %>%
  ...
  combined_LS_AUG <- full_join(comparing_genes_stages_5LS, 
...
GetGeneDatamatrix5LS_non_A <- function(gene, dataset, hdf5file, gffdf, nnt_gene) {
  data_mat_all <- GetGeneDatamatrix(gene, dataset, hdf5file)
  n_left5 <- Get5LSstart(gene, gffdf) # column to start from (5'end)
...
LS5_table <- function(gene) {
  lapply(gene,
         function(gene)
           GetGeneDatamatrix5LS_non_A(gene,
                                       dataset,
                                       hdf5file,
                                       gffdf =gff_df,
                                       nnt_gene = 50)
