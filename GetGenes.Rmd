...
``` {r}
#Visualize the original GFF created with gff_df <- readGFFAsDf(orf_gff_file)
head(gff_df)
...
Original function, which iterates over many genes used lapply: 
``` {r}
example <-lapply(test_orfs,
                     function(gene)
...
Thoughts:
-we probably don't want to use lapply as it outputs a list; we want a tibble.Should we modify it so it outputs one big tibble for all genes or individual tibble for each gene?
